/*
 * BDN Credit Decisioning — credentials proxy (backend-for-frontend)
 *
...
const CONFIG = {
  ...
  workflowName: process.env.WORKFLOW_NAME || 'credit_scoring_approval',
  workflowVersion: process.env.WORKFLOW_VERSION || '1',
  reviewTaskRef: process.env.REVIEW_TASK_REF || 'manual_review_ref',
...
async function getApplication(id) {
  ...
  // Is the workflow currently paused on the manual-review WAIT task?
...
server.listen(CONFIG.port, () => {
  console.log(`\nBRI Credit Decisioning UI  ->  http://localhost:${CONFIG.port}`);
